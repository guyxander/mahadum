...
export type DashboardData={generatedAt:number;userId:string;name:string;email:string;profile:Row|null;payoutAccount:Row|null;payoutBanks:PaystackBank[];categories:Row[];courses:Row[];marketplaceCourses:Row[];enrollments:Row[];progress:Row[];certificates:Row[];reviews:Row[];users:Row[];creators:Row[];payments:Row[];refunds:Row[];payouts:Row[];ledger:Row[];affiliates:Row[];referrals:Row[];audit:Row[];settings:Row[]};
...
export async function loadDashboard(role:string){
  ...
  const base:DashboardData={generatedAt:Date.now(),userId:user.id,name:String(user.user_metadata?.full_name||user.email?.split("@")[0]||"Member"),email:user.email||"",profile:null,payoutAccount:null,payoutBanks:[],categories:[],courses:[],marketplaceCourses:[],enrollments:[],progress:[],certificates:[],reviews:[],users:[],creators:[],payments:[],refunds:[],payouts:[],ledger:[],affiliates:[],referrals:[],audit:[],settings:[]};
  const [{data:profile},{data:categories},{data:affiliateRows},{data:referralRows},{data:publishedCourses}]=await Promise.all([client.from("profiles").select("*").eq("id",user.id).maybeSingle(),client.from("categories").select("*").order("sort_order"),client.from("affiliates").select("*").eq("user_id",user.id),client.from("referrals").select("*").eq("affiliate_id",user.id).order("attributed_at",{ascending:false}),client.from("courses").select("id,title,slug,short_description,thumbnail_path,price_minor,currency").eq("status","published").order("published_at",{ascending:false})]);
  base.profile=profile as Row|null;base.categories=rows(categories);base.name=String(profile?.full_name||base.name);base.affiliates=rows(affiliateRows);base.referrals=rows(referralRows);base.marketplaceCourses=rows(publishedCourses);
  if(role==="creator"){
    const results=await Promise.all([client.from("courses").select("*, categories(name), course_modules(id,title,position,lessons(id,title,description,video_url,duration_seconds,position,is_free_preview))").eq("creator_id",user.id).order("created_at",{ascending:false}),client.from("ledger_entries").select("*, payments(tx_ref,courses(title))").eq("owner_id",user.id).order("created_at",{ascending:false}),client.from("payouts").select("*").eq("user_id",user.id).order("requested_at",{ascending:false}),client.from("payout_accounts").select("*").eq("user_id",user.id).maybeSingle()]);
    [base.courses,base.ledger,base.payouts]=results.slice(0,3).map(x=>rows(x.data));base.payoutAccount=(results[3].data as Row|null)||null;
    if(process.env.PAYSTACK_SECRET_KEY)base.payoutBanks=await listPaystackBanks(process.env.PAYSTACK_SECRET_KEY);
