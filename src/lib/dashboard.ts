...
export type DashboardData={userId:string;name:string;email:string;profile:Row|null;categories:Row[];courses:Row[];enrollments:Row[];progress:Row[];certificates:Row[];users:Row[];creators:Row[];payments:Row[];refunds:Row[];payouts:Row[];ledger:Row[];affiliates:Row[];audit:Row[]};
...
export async function loadDashboard(role:string){
  ...
  const base:DashboardData={userId:user.id,name:String(user.user_metadata?.full_name||user.email?.split("@")[0]||"Member"),email:user.email||"",profile:null,categories:[],courses:[],enrollments:[],progress:[],certificates:[],users:[],creators:[],payments:[],refunds:[],payouts:[],ledger:[],affiliates:[],audit:[]};
