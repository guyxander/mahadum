...
export default async function CoursePlayer({
  ...
  const { data: modules } = await client
    .from("course_modules")
    .select(
      "id,title,position,lessons(id,title,description,video_url,duration_seconds,position,module_id)",
    )
    .eq("course_id", course)
    .order("position");
  const lessons = (
    ...
    ) as (Lesson & { moduleTitle: string; modulePosition: number })[]
  ).sort(
    (a, b) => a.modulePosition - b.modulePosition || a.position - b.position,
  );
  const active = lessons.find((item) => item.id === lesson);
  if (!active) notFound();
  ...
  const current = lessons.indexOf(active);
  const next = lessons[current + 1];
  const previous = lessons[current - 1];
  ...
  return (
    <main className="player">
      <header>
        ...
        <div>
          {String(
            (enrollment.courses as { title?: string } | null)?.title ||
              "Course",
          )}{" "}
          <span>{percent}% complete</span>
        </div>
        <Link href="/dashboard/learner/my-learning">Exit</Link>
      </header>
      <div className="player-grid">
        <details className="player-mobile-curriculum">
          ...
          {lessons.map((item, index) => (
            ...
            >
              ...
              <div>
                <b>{item.title}</b>
                <small>
                  {Math.ceil((item.duration_seconds || 0) / 60)} min
                </small>
              </div>
            </Link>
          ))}
        ...
        <section>
          ...
          <div className="lesson-content">
            <span className="overline">
              {active.moduleTitle} · Lesson {active.position + 1}
            </span>
            <h1>{active.title}</h1>
            <p>
              {active.description || "No lesson notes have been added yet."}
            </p>
            <div className="lesson-actions">
              {previous ? (
                <Link
                  className="outline-button"
                  href={`/learn/${course}/${previous.id}`}
                >
                  ← Previous
                </Link>
              ) : (
              ...
              <form action={markLessonComplete}>
                <input
                  ...
                  value={enrollment.id}
                />
                <input type="hidden" name="lesson_id" value={active.id} />
              ...
              {next && completed.has(active.id) && (
                <Link
                  className="text-link"
                  href={`/learn/${course}/${next.id}`}
                >
                  Next →
                </Link>
              )}
