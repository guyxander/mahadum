...
export function AppShell({ role, section, name, children }: { role: Role; section: string; name:string; children: React.ReactNode }) {
  const alternateWorkspace = role === "creator"
    ? { href: "/dashboard/learner/my-learning", label: "Switch to learning" }
    : role === "learner"
      ? { href: "/dashboard/creator/overview", label: "Switch to Creator Hub" }
      : null;
  const navigation = nav[role].map((item) => {
    const slug = item.toLowerCase().replaceAll(" ", "-");
    return <Link className={section === slug ? "active" : ""} href={`/dashboard/${role}/${slug}`} key={item}><span>{item.slice(0,1)}</span>{item}</Link>;
  });
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <details className="dashboard-mobile-menu"><summary aria-label="Open dashboard menu"><span aria-hidden="true" /></summary><nav aria-label="Dashboard navigation">{nav[role].map((item) => { const slug = item.toLowerCase().replaceAll(" ", "-"); return <Link className={section === slug ? "active" : ""} href={`/dashboard/${role}/${slug}`} key={item}>{item}</Link>; })}{alternateWorkspace&&<Link href={alternateWorkspace.href}>{alternateWorkspace.label}</Link>}<Link href="/">Marketplace</Link><form action={signOut}><button>Sign out</button></form></nav></details>
      <div className="workspace-label">{role === "creator" ? "Creator Hub" : role === "admin" ? "Admin console" : "Learning space"}</div>
      <nav>{navigation}</nav>
      <div className="sidebar-foot">{alternateWorkspace&&<Link href={alternateWorkspace.href}>{alternateWorkspace.label}</Link>}<Link href="/">← Marketplace</Link><form action={signOut}><button>Sign out</button></form></div>
    </aside>
