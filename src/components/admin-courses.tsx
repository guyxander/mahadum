...
export function AdminCourses({courses,categories}:{courses:Row[];categories:Row[]}){
  ...
  const counts={total:courses.length,published:courses.filter(x=>x.status==="published").length,draft:courses.filter(x=>x.status==="draft").length,review:courses.filter(x=>x.status==="in_review").length};
  return <div className="dashboard-page admin-courses-page">
    ...
    <div className="metric-grid admin-course-metrics"><article className="metric-card"><span>Total courses</span><strong>{counts.total}</strong></article><article className="metric-card"><span>Published</span><strong>{counts.published}</strong></article><article className="metric-card"><span>Drafts</span><strong>{counts.draft}</strong></article><article className="metric-card"><span>Awaiting review</span><strong>{counts.review}</strong></article></div>
    <section className="panel admin-courses-panel">
      <div className="admin-course-tools"><label><span>Search courses</span><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Title or creator"/></label><label><span>Status</span><select value={status} onChange={event=>setStatus(event.target.value)}><option value="all">All statuses</option><option value="published">Published</option><option value="in_review">Awaiting review</option><option value="draft">Draft</option><option value="rejected">Rejected</option><option value="archived">Archived</option></select></label><label><span>Category</span><select value={category} onChange={event=>setCategory(event.target.value)}><option value="all">All categories</option>{categories.map(item=><option key={text(item.id)} value={text(item.id)}>{text(item.name)}</option>)}</select></label><label><span>Sort</span><select value={sort} onChange={event=>setSort(event.target.value)}><option value="newest">Newest first</option><option value="title">Course title</option><option value="enrollments">Most enrolled</option></select></label></div>
