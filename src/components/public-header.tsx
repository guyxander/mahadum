...
export async function PublicHeader({courseBack=false}:{courseBack?:boolean}) {
  ...
  const {data:profile}=user&&client?await client.from("profiles").select("role").eq("id",user.id).maybeSingle():{data:null};
  const role=profile?.role==="admin"||profile?.role==="creator"?profile.role:"learner";
